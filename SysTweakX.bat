...
:RemoveOverlay & call :RemoveUpdateFiles
    ...
    if /i "%arg%"=="/ClassicContextMenu" call :VisualOverlay & call :ClassicContextMenu
    if /i "%arg%"=="/LeftTaskbar" call :VisualOverlay & call :LeftTaskbar
...
:BlockTelemetry
REM Блокировка телеметрии (hosts)
...
:ClassicContextMenu
REM Классическое контекстное меню (Win10)
	reg add "HKCU\Software\Classes\CLSID\{86ca1aa0-34aa-4e8b-a509-50c905bae2a2}\InprocServer32" /f /ve >nul 2>&1
	exit /b

:LeftTaskbar
REM Панель задач по левому краю
